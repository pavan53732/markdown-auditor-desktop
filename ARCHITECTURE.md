...
- deterministic local rule-engine enforcement for duplicate headings, broken cross-references, RFC2119 misuse, ambiguous requirement wording, duplicated requirements, requirement-strength conflicts, missing terminal states, rollback gaps, missing workflow triggers, execution idempotency gaps, workflow ordering/terminal-state issues, execution-order indeterminism, governance checkpoint gaps, governance bypass patterns, compliance-gate omissions, policy-priority conflicts, enforcement-path gaps, audit-trail requirements, authority-delegation ambiguity, execution-owner boundary clarity, policy-enforcement-point gaps, control-plane override guard gaps, retry/backoff policy gaps, deadlock/livelock risk articulation gaps, deterministic replay requirements, execution-determinism gaps, output-determinism gaps, commit-hash binding, transition-determinism failures, transition non-determinism, concurrency-model ambiguity, input-determinism gaps, state-transition preconditions/postconditions, task-graph DAG/cycle validation, missing prerequisite-node detection, dependency-order checks, optional-vs-required dependency ambiguity, redundant dependency detection, dependency-ownership detail checks, dependency ownership/lifecycle checks, dependency-version ambiguity, transitive dependency conflicts, disconnected task-node detection, priority propagation checks, conflicting workflow triggers, incomplete execution branches, execution-path closure gaps, path dependency cycles, path state-preservation gaps, conditional execution indeterminism, execution branching-logic validation, missing approval gates, untraceable change detection, governance ownership ambiguity, missing fail-safe conditions, override ambiguity, unauthorized agent writes, decision-authority ambiguity, parallel resource ordering checks, snapshot-isolation enforcement, memory conflict-resolution enforcement, PSG write-atomicity enforcement, read/write consistency enforcement, tool sandbox-isolation checks, tool side-effect validation, forbidden tool-write detection, tool-result validation, duplicate-context assembly detection, context overflow handling, retrieval validation, retrieval relevance ranking validation, intent ambiguity detection, change-scope boundary checks, user-intent consistency validation, interaction conflict detection, reasoning evidence binding, reasoning trace-completeness checks, uncertainty-propagation checks, evidence-free escalation detection, unresolved UI-state mappings, undefined reused identifiers, unresolved glossary bindings, API return/error/idempotency/rate-limit/auth contract gaps, terminology-registry gaps, malformed terminology registries, symbol inconsistency, state-space definition gaps, and input/output contract determinism
...
- Windows packaging for both portable and installer distributions, with env-driven code-signing support for signed release builds
...
- UI-shell rendering over extracted runtime controllers and the `useAuditWorkbench.js` state/controller hook
...
- export generation and workbench/session orchestration (JSON, Markdown, CSV, history/session workflows) using dedicated `exportFormats.js`, `workbenchController.js`, `auditOrchestrator.js`, `pipelineController.js`, `resultAggregator.js`, `executionStateController.js`, `useAuditWorkbench.js`, and extracted audit-pipeline helpers for runtime coverage accounting and post-merge escalation
...
|       |-- useAuditWorkbench.js (Renderer state, actions, and controller hook)
...
|   `-- icon.png
|-- dist/
`-- dist-electron-v4/ (generated locally for release packaging; not committed)
...
`src/App.jsx` is the UI-shell composition layer for:
...
- renderer state/action orchestration delegated through `useAuditWorkbench.js`
...
| `SummaryDashboard.jsx` | severity summary plus runtime depth cards for graph groups, rule issues, owned-detector reconciliation, mesh passes, focus hits, reuse, merge warnings, and layer-level detector coverage |
| `DiagnosticsPanel.jsx` | results/error diagnostics rendering for enrichment, coverage, agent failures, and mesh reconciliation |
...
- env-driven Windows code signing is configured through `electron-builder.config.cjs`, `scripts/windowsSigningSupport.cjs`, and `npm run dist:signed`; local builds remain unsigned unless certificate or Azure Trusted Signing environment variables are present
- Large packaged `.exe` artifacts are intended for GitHub Releases instead of repository storage
