...
- **Deterministic Rule Expansion To 83 Rules**: Expanded the local rule engine with additional receipt-backed rules for redundant dependency declarations, detailed dependency-ownership ambiguity, conflicting workflow triggers, incomplete execution branches, dead-end workflows, conditional ambiguity, execution-path closure gaps, path dependency cycles, path state-preservation gaps, conditional execution indeterminism, execution branching logic, missing approval gates, untraceable changes, governance ownership ambiguity, missing fail-safe conditions, override ambiguity, unauthorized agent writes, transition non-determinism, concurrency-model ambiguity, input-determinism gaps, and decision-authority ambiguity.
...
- **Verification Coverage**: The local suite now covers 240 tests across 24 files, including Windows signing-config validation, layer-level coverage reporting, extracted `useAuditWorkbench` coverage, extracted diagnostics rendering support, and the earlier deterministic rule-engine, pipeline, and analysis-mesh coverage.
