...
- Deterministic local rule engine with 99 receipt-backed rules for broken heading hierarchy, orphan sections, duplicate headings, broken cross-references, RFC2119 misuse, ambiguous requirement wording, duplicated requirements, conflicting requirement strength, missing terminal states, missing rollback paths, missing workflow triggers, execution idempotency gaps, workflow ordering gaps, workflow exit criteria, execution-order indeterminism, governance checkpoint gaps, governance bypass patterns, compliance-gate omissions, policy-priority conflicts, enforcement-path gaps, audit-trail omissions, authority-delegation ambiguity, execution-owner boundary gaps, policy-enforcement-point gaps, control-plane override guard gaps, retry/backoff policy gaps, deadlock/livelock risk articulation gaps, deterministic replay requirement gaps, execution-determinism gaps, output-determinism gaps, commit-hash binding gaps, transition-determinism failures, state-transition precondition/postcondition gaps, task-graph DAG/cycle detection, missing prerequisite-node detection, dependency-order validation, optional-vs-required dependency confusion, dependency ownership/lifecycle gaps, dependency version ambiguity, transitive dependency conflicts, disconnected-node detection, priority propagation checks, redundant dependency declarations, detailed dependency-ownership ambiguity checks, conflicting workflow triggers, incomplete execution branches, dead-end workflows, conditional ambiguity detection, execution-path closure gaps, path dependency cycles, path state-preservation gaps, conditional execution indeterminism, execution branching-logic validation, missing approval gates, untraceable change detection, governance ownership ambiguity, missing fail-safe conditions, override ambiguity checks, unauthorized agent-write detection, transition non-determinism, concurrency model ambiguity, input determinism gaps, decision-authority ambiguity, parallel resource ordering gaps, snapshot isolation gaps, memory conflict-resolution gaps, PSG write atomicity gaps, read/write consistency gaps, tool sandbox-isolation boundary gaps, tool side-effect validation gaps, forbidden direct tool writes, tool result-validation gaps, duplicate context-assembly gaps, context overflow/truncation gaps, retrieval validation/relevance gaps, intent ambiguity detection, change-scope boundary checks, user-intent consistency checks, interaction conflict detection, reasoning evidence-binding gaps, reasoning trace-completeness gaps, uncertainty-propagation failures, evidence-free escalation, unresolved UI-to-system-state mappings, undefined identifiers, unresolved glossary bindings, API return/error/idempotency/rate-limit/auth contract gaps, terminology registry gaps, malformed terminology registries, symbol inconsistency, state-space definition gaps, and input/output contract determinism, with per-detector checked/clean/hit execution receipts
...
- Layer-level deterministic coverage dashboard: the summary UI and Markdown exports now include per-layer touched/checked/model-backed coverage snapshots instead of only whole-catalog totals
...
- Further-decomposed runtime orchestration: `auditOrchestrator.js`, `pipelineController.js`, `resultAggregator.js`, `executionStateController.js`, `agentMeshRuntime.js`, `sessionService.js`, `useAuditWorkbench.js`, and `workbenchController.js` now own most non-UI pipeline logic, while `App.jsx` acts as a thinner UI shell with `DiagnosticsPanel.jsx` handling diagnostics rendering
