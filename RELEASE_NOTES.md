...
- **Layer-Level Coverage Dashboard**: Runtime summaries, diagnostics reloads, and Markdown exports now include per-layer touched, locally checked, model-backed, untouched, and deterministic-vs-model-driven coverage snapshots instead of only whole-catalog totals.
- **Near-UI-Shell App Composition**: `src/App.jsx` is now a thinner UI shell, with `useAuditWorkbench.js` carrying most renderer-side state/actions and `DiagnosticsPanel.jsx` owning the large diagnostics rendering block.
- **99 Deterministic Local Rules**: The local rule engine now covers 99 receipt-backed checks, including the newest memory-world-model, tool-execution-safety, context-orchestration, and reasoning-integrity expansions on top of the recent dependency, execution-path, governance, control-plane, and interaction-intelligence work.
...
- **Deterministic Coverage Metrics**: Runtime summaries, diagnostics, exports, and session reloads now show deterministic-vs-model-driven catalog coverage percentages in addition to checked/touched detector counts.
- **Further Runtime Modularization**: `auditOrchestrator.js`, `pipelineController.js`, `resultAggregator.js`, and `executionStateController.js` now carry most staged analysis orchestration, result shaping, and progress-state logic that used to live inline in `App.jsx`.
- **More Deterministic Formal Rules**: The local rule engine now also catches missing prerequisite-node references, optional-vs-required dependency ambiguity, compliance-gate omissions, execution-determinism gaps, and transition-determinism failures before model synthesis.
...
- **Prompt Payload Efficiency**: The live 8-agent runtime continues to use scoped prompt compaction, and the local verification suite now covers 244 tests across 24 files.
...
- **244 Automated Tests / 24 Test Files**: The local verification suite now covers deterministic trust-signal enrichment, proof-status/trust-basis validation, trust-tier/source-priority ordering, proof-aware severity gating, extracted report-format generation, extracted audit-pipeline coverage/escalation/merge behavior, extracted audit-orchestrator / pipeline-controller behavior, extracted agent-mesh runtime behavior, extracted session-service normalization behavior, extracted workbench-controller behavior, extracted `useAuditWorkbench` behavior, layer-level coverage reporting, deeper governance/execution/dependency deterministic-rule coverage, new deterministic memory/tool/context/reasoning coverage, typed proof-chain generation and fallback generation, the rule-engine, receipt-backed detector coverage, richer graph/evidence paths, unified layer numbering, explicit agent-ownership reconciliation, adaptive timeout handling, hardened timeout normalization, deterministic task-graph validation, deterministic interaction-intelligence validation, Windows signing-config validation, and analysis-mesh runtime validation.
