...
- **Prompt Payload Efficiency**: The live 8-agent runtime continues to use scoped prompt compaction, and the local verification suite now covers 197 tests across 18 files.
...
- **Deterministic Local Rule Engine**: The runtime now evaluates a local rule layer before agent synthesis, catching broken heading hierarchy, orphan sections, duplicate headings, broken cross-references, RFC2119 misuse, rollback gaps, workflow ordering gaps, missing workflow exit criteria, undefined reused identifiers, and unresolved glossary bindings without depending on the model alone, and it emits per-detector checked/clean/hit receipts.
...
- **197 Automated Tests / 18 Test Files**: The local verification suite now covers typed proof-chain generation and proof-chain fallback generation in addition to the rule-engine, receipt-backed detector coverage, richer graph/evidence paths, unified layer numbering, explicit agent-ownership reconciliation, adaptive timeout handling, hardened timeout normalization, and analysis-mesh runtime validation.
