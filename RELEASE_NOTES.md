...
- **Prompt Payload Efficiency**: The live 8-agent runtime continues to use scoped prompt compaction, and the local verification suite now covers 184 tests across 17 files.
...
- **Deterministic Local Rule Engine**: The runtime now evaluates a local rule layer before agent synthesis, catching duplicate headings, broken cross-references, RFC2119 misuse, rollback gaps, workflow ordering gaps, missing workflow exit criteria, undefined reused identifiers, and unresolved glossary bindings without depending on the model alone.
...
- **184 Automated Tests / 17 Test Files**: The local verification suite now covers the rule-engine and richer graph/evidence paths in addition to the prior indexing, validation, history, diagnostics, and benchmark flows.
