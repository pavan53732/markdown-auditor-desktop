...
- **83 Deterministic Local Rules**: The local rule engine now covers 83 receipt-backed checks, including the newest dependency, execution-path, governance, control-plane, and interaction-intelligence expansions.
...
- **Prompt Payload Efficiency**: The live 8-agent runtime continues to use scoped prompt compaction, and the local verification suite now covers 240 tests across 24 files.
...
- **240 Automated Tests / 24 Test Files**: The local verification suite now covers deterministic trust-signal enrichment, proof-status/trust-basis validation, trust-tier/source-priority ordering, proof-aware severity gating, extracted report-format generation, extracted audit-pipeline coverage/escalation/merge behavior, extracted audit-orchestrator / pipeline-controller behavior, extracted agent-mesh runtime behavior, extracted session-service normalization behavior, extracted workbench-controller behavior, extracted `useAuditWorkbench` behavior, layer-level coverage reporting, deeper governance/execution/dependency deterministic-rule coverage, typed proof-chain generation and fallback generation, the rule-engine, receipt-backed detector coverage, richer graph/evidence paths, unified layer numbering, explicit agent-ownership reconciliation, adaptive timeout handling, hardened timeout normalization, deterministic task-graph validation, deterministic interaction-intelligence validation, Windows signing-config validation, and analysis-mesh runtime validation.
